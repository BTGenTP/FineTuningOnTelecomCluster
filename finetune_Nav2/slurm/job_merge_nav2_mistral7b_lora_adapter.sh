#!/bin/bash
#SBATCH --job-name=nav2_merge_mistral7b
#SBATCH --output=nav2_merge_mistral7b_%j.out
#SBATCH --error=nav2_merge_mistral7b_%j.err
...
WORK_DIR="${WORK_DIR:-$HOME/models/Mistral-7B-Instruct-v02-Lora-Nav2-BT}"
VENV_DIR="${VENV_DIR:-$HOME/venvs/nav4rail_nav2_merge}"

BASE_ID="${BASE_ID:-mistralai/Mistral-7B-Instruct-v0.2}"
# ADAPTER_ID="${ADAPTER_ID:-mlatoundji/Mistral-7B-Instruct-v02-Lora-Nav2-BT}"
MERGED_ID="${MERGED_ID:-mlatoundji/Mistral-7B-Instruct-v0.2-Nav2BT-merged}"
...
# print(f"[job] base_id={base_id} -> {base_dir}")
# print(f"[job] adapter_id={adapter_id} -> {adapter_dir}")
...
# Merge is RAM-heavy; force CPU and allow offload to disk to reduce peak RSS.

...
from finetune_Nav2_XML.constraints.xml_prefix_fn import build_prefix_allowed_tokens_fn
...
from finetune_Nav2_XML.train.model_registry import MODELS
from finetune_Nav2_XML.train.prompting import build_chat_messages, build_mistral_inst_prompt, build_phi2_prompt
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="HF eval: model/adpater -> BT XML -> validate -> runs/.")
    ...
    p.add_argument("--reference-dir", type=str, default=None, help="Optional reference BT dir for regex allowlist.")
    p.add_argument("--n", type=int, default=5)
    p.add_argument("--max-new-tokens", type=int, default=1024)
    p.add_argument("--temperature", type=float, default=0.0)
    p.add_argument("--constrained", choices=["off", "regex"], default="off")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    prefix_fn = None
    if args.constrained == "regex":
        prefix_fn, _regex = build_prefix_allowed_tokens_fn(
            tokenizer,
            catalog,
            reference_dir=(args.reference_dir or (Path(__file__).resolve().parents[1] / "reference_behavior_trees")),
        )
    ...
    for row in rows:
        ...
        experiment = {
            ...
            "generator": {"mode": "fail-fast", "constraints": {"enabled": args.constrained != "off", "kind": str(args.constrained)}},

...
from finetune_Nav2_XML.train.model_registry import MODELS, ModelSpec
...
def build_text_row(spec: ModelSpec, tokenizer, catalog: Dict[str, Any], mission: str, xml: str) -> str:
    """
...
def load_dataset_jsonl(path: Path, spec: ModelSpec, tokenizer, catalog: Dict[str, Any]) -> Dataset:
    ...
    for r in rows:
        ...
        xml = str(r.get("xml") or "").strip()
        if not mission or not xml:
            continue
        text = build_text_row(spec, tokenizer, catalog, mission, xml)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="QLoRA SFT: mission -> BT XML (Nav2 XML-direct).")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    out_dir = (
        ...
        if args.output_dir
        else Path(__file__).resolve().parents[1] / "outputs" / f"nav2_xml_{spec.key}_lora"
    )
